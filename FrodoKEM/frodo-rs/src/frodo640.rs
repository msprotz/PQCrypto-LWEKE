...
pub fn crypto_kem_dec_Frodo640(ss: &mut [u8], ct: &[u8], sk: &[u8]) -> i32
{
  ...
  let mut BBp: Aligned<A32, [u16; 5120]> = Aligned([0u16; 5120usize]);
  let mut Sp: Aligned<A32, [u16; 10304]> = Aligned([0u16; 10304usize]);
  let ct_c1: (&[u8], &[u8]) = ct.split_at(0usize);
  let ct_c2: (&[u8], &[u8]) = (ct_c1.1).split_at(9600usize);
  let salt: (&[u8], &[u8]) = (ct_c2.1).split_at(120usize);
  let sk_s: (&[u8], &[u8]) = sk.split_at(0usize);
  let pk_seedA: (&[u8], &[u8]) = (sk_s.1).split_at(16usize);
  let pk_b: (&[u8], &[u8]) = (pk_seedA.1).split_at(16usize);
  let sk_S0: (&[u8], &[u8]) = (pk_b.1).split_at(9600usize);
  let sk_S: &[u16] = crate::scylla_glue::scylla_u16_of_u8(sk_S0.1);
  let sk_pkh: (&[u8], &[u8]) = (sk_S0.1).split_at(10240usize);
  let mut S: [u16; 5120] = [0u16; 5120usize];
  let mut G2in: Aligned<A32, [u8; 64]> = Aligned([0u8; 64usize]);
  ...
  let kprime: (&[u8], &[u8]) = (seedSEprime.1).split_at(32usize);
  ...
  let Ep0: (&mut [u16], &mut [u16]) = (Sp0.1).split_at_mut(5120usize);
  ...
  crate::util::ct_select(&mut Fin[9752usize..], kprime.1, pk_seedA.0, 16usize, selector);
...
pub fn crypto_kem_enc_Frodo640(ct: &mut [u8], ss: &mut [u8], pk: &[u8]) -> i32
{
  ...
  let ct_c2: (&mut [u8], &mut [u8]) = (ct_c1.1).split_at_mut(9600usize);
  let mut B: [u16; 5120] = [0u16; 5120usize];
  let mut V: [u16; 64] = [0u16; 64usize];
  let mut C: [u16; 64] = [0u16; 64usize];
  let mut Bp: Aligned<A32, [u16; 5120]> = Aligned([0u16; 5120usize]);
  let mut Sp: Aligned<A32, [u16; 10304]> = Aligned([0u16; 10304usize]);
  let mut G2in: Aligned<A32, [u8; 64]> = Aligned([0u8; 64usize]);
  let mut G2out: [u8; 48] = [0u8; 48usize];
  let mut Fin: [u8; 9768] = [0u8; 9768usize];
  ...
  let k: (&[u8], &[u8]) = (seedSE.1).split_at(32usize);
  ...
  ((&mut Fin[0usize..])[0usize..9752usize]).copy_from_slice(&ct[0usize..9752usize]);
  ((&mut Fin[9752usize..])[0usize..16usize]).copy_from_slice(&k.1[0usize..16usize]);
  crate::fips202::shake128(ss, 16u64, &Fin, 9752i32.wrapping_add(16i32) as u64);
  crate::util::clear_bytes(
...
pub fn crypto_kem_keypair_Frodo640(pk: &mut [u8], sk: &mut [u8]) -> i32
{
  ...
  let sk_S: (&mut [u8], &mut [u8]) = (sk_pk.1).split_at_mut(9616usize);
  let sk_pkh: (&mut [u8], &mut [u8]) = (sk_S.1).split_at_mut(10240usize);
  let mut B: Aligned<A32, [u16; 5120]> = Aligned([0u16; 5120usize]);
  let mut S: Aligned<A32, [u16; 10240]> = Aligned([0u16; 10240usize]);
  ...
  let randomness_z: (&[u8], &[u8]) = (randomness_seedSE.1).split_at(32usize);
  crate::fips202::shake128(pk_seedA.1, 16u64, randomness_z.1, 16u64);
  shake_input_seedSE[0usize] = 95u8;
  ((&mut shake_input_seedSE[1usize..])[0usize..2i32.wrapping_mul(16i32) as usize]).copy_from_slice(
    &randomness_z.0[0usize..2i32.wrapping_mul(16i32) as usize]
  );
  crate::fips202::shake128(
    crate::scylla_glue::scylla_u8_of_u16_mut(&mut S[..]),
  ...
  (sk_pk.0[0usize..16usize]).copy_from_slice(&randomness_seedSE.0[0usize..16usize]);
...
pub fn frodo_mul_add_as_plus_e(out: &mut [u16], s: &[u16], e: &[u16], seed_A: &[u8]) -> i32
{
  ...
  let mut j: i32;
  let mut k: i32;
  let mut a_row: Aligned<A32, [i16; 2560]> = Aligned([0i16; 2560usize]);
  {
    i = 0i32;
    while
...
pub fn frodo_mul_add_sa_plus_e(out: &mut [u16], s: &[u16], e: &mut [u16], seed_A: &[u8]) -> i32
{
  ...
  let mut q: i32;
  let mut p: i32;
  let mut A: Aligned<A32, [u16; 5120]> = Aligned([0u16; 5120usize]);
  let mut aes_key_schedule: [u8; 176] = [0u8; 176usize];
  crate::aes::AES128_load_schedule(seed_A, &mut aes_key_schedule);
  let mut Ainit: Aligned<A32, [u16; 5120]> = Aligned([0u16; 5120usize]);
  {
    j = 0i32;
    while
    j < 640i32
